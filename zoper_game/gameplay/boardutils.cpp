    ...
    function<bool(const vector2dst&, const Direction&)> updatePredicate)
{
    ...
    for_each_coordinate(startPosition, direction,
                        [&board_size, updatePredicate](const vector2dst& v, const Direction& d) {
                            return (board_size.x > v.x) && (board_size.y > v.y) && updatePredicate(v,d);
                        });
}
