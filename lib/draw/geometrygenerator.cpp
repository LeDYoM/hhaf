...
namespace lib
{
	namespace draw
	{
		BasicVertexArray GeometryGenerator::generateQuad()
		{
			constexpr u32 nPoints = 4;
			constexpr u32 nVertex = nPoints + 2;


			BasicVertexArray vertices{ Vertex{vector2df(0, 0),color, {0,0}} }; // + 2 for center and repeated first point
			...
			vertices[0].position.x = size.x / 2;
			vertices[0].position.y = size.y / 2;
		...
		Rectf32 GeometryGenerator::generateShape(const vector2df & size, const u32 granularity)
		{
		...
		Rectf32 GeometryGenerator::generateText(const sptr<Font> &font, std::string str,
