HTPS_PRAGMA_ONCE
#ifndef HTYPES_VECTOR4D_INCLUDE_HPP
#define HTYPES_VECTOR4D_INCLUDE_HPP
...
{
public:
    static constexpr htps::u16 kNumElements{4U};
    using value_type = T;
...
template <typename T>
constexpr str& operator<<(str& os, vector4d<T> const& v)
{
    os << "{" << v.x << "," << v.y << "," << v.z << "," << v.w << "}";
    return os;
}
...
template <typename T>
constexpr str& operator>>(str& is, vector4d<T> const& v)
{
    return is;
}

static_assert(std::is_standard_layout_v<vector4d<u8>>,
              "vector4du8 has not standard layout");
static_assert(std::is_standard_layout_v<vector4d<f64>>,
              "vector4df64 has not standard layout");

static_assert(std::is_trivial_v<vector4d<f32>>, "vector4df32 is not trivial");

using vector4du8  = vector4d<u8>;
using vector4ds8  = vector4d<s8>;
using vector4du16 = vector4d<u16>;
using vector4ds16 = vector4d<s16>;
using vector4du32 = vector4d<u32>;
using vector4ds32 = vector4d<s32>;
using vector4df   = vector4d<f32>;
using vector4dd   = vector4d<f64>;
using vector4dst  = vector4d<size_type>;
}  // namespace htps
