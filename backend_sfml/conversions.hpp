...
template <typename T>
constexpr const mtps::Rect<T> from_sft_type(const sf::Rect<T> &rect) noexcept { return mtps::Rect<T>{rect.left, rect.top, rect.width, rect.height}; }

template <typename T>
constexpr const sf::Vector2<T> to_sf_type(const mtps::vector2d<T> &v) noexcept { return sf::Vector2<T>{v.x, v.y}; }
