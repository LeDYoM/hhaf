...
Rectf32 TTFont::getBounds(const u32 codePoint, const u32 characterSize) const
{
    return from_sft_type<f32>(
        m_font->getGlyph(codePoint, characterSize, false).bounds);
}
                                 ...
                                 const u32 characterSize) const
{
    return from_sft_type<s32>(
        m_font->getGlyph(codePoint, characterSize, false).textureRect);
}

f32 TTFont::getAdvance(const u32 codePoint, const u32 characterSize) const
{
    return static_cast<f32>(
        m_font->getGlyph(codePoint, characterSize, false).advance);
}
