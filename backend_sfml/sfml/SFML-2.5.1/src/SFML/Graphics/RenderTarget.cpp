...
////////////////////////////////////////////////////////////
void RenderTarget::mydraw(const VertexBuffer& vertexBuffer, std::size_t firstVertex,
                        std::size_t vertexCount, const RenderStates& states)
{
    // VertexBuffer not supported?
    if (!VertexBuffer::isAvailable())
...
void RenderTarget::mysetupDraw(bool useVertexCache, const RenderStates& states)
{
    // First set the persistent OpenGL states if it's the very first call
    if (!m_cache.glStatesSet)
        resetGLStates();
...
void RenderTarget::mycleanupDraw(const RenderStates& states)
{
    // Unbind the shader, if any
//    if (states.shader)
//        applyShader(NULL);
