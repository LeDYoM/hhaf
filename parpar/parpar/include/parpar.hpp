...
public:
    ...
    using OptionParameter           = tps::pair<str_t, str_t>;
    ...
    SyntaxParserErrorCodes errorAtParameter(
        tps::size_type const position) const noexcept
    {
     ...
     */
    tps::size_type numPositionalParameters() const noexcept
    {
     ...
     * @return size_type The value.
     */
    tps::size_type numSwitchParameters() const noexcept
    {
     ...
     * @return size_type The value.
     */
    tps::size_type numOptionParameters() const noexcept
    {
     ...
     * @return size_type The value
     */
    tps::size_type numParameters() const noexcept
    {
     ...
     */
    bool hasParameters() const noexcept { return !(emptyParameters()); }

    /**
     * @brief Get the total number of syntax errors detected
     * @return The value
     */
    tps::size_type numSyntaxErrors() const
    {
     ...
     */
    str_t positionalParameterAt(tps::size_type const position) const noexcept
    {
     ...
     */
    bool positionalParameterExists(str_t const& poPar) const
    {
    ...
    bool switchExists(str_t const& swPar) const
    {
    ...
    bool optionExists(str_t const& opPar) const
    {
    ...
    tps::pair<bool, str_t> optionValue(str_t const& opPar) const
    {
        ...
        return iterator == option_parameters.cend()
            ...
            : tps::pair{true, str_t{iterator->second}};
    }

    str_t optionValueOrDefault(str_t const& opPar, str_t const& def) const
    {
...
private:
    ...
    using COptionResult =
        tps::pair<SyntaxParserErrorCodes, tps::pair<str_t, str_t>>;
        ...
        for (decltype(argv.size()) i{1}; i < argv.size(); ++i)
        {
            ...
            if (!param.empty())
            {
                switch (getParameterType(param))
                {
                    ...
                    {
                        positional_parameters_.push_back(tps::move(param));
