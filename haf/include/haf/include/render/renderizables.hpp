...
class Renderizables final
{
public:
    void setSceneNode(core::rptr<scene::SceneNode> scene_node) noexcept;
    ...
    void removeRenderizable(core::sptr<Renderizable> const& element);
    void clearRenderizables();

    template <typename T>
    constexpr void for_each_node_as(
        core::function<void(core::sptr<T> const&)> action) const
    {
        for_each_node([&action](core::sptr<Renderizable> const& node) {
            if (auto const tnode{core::dynamic_pointer_cast<T>(node)})
            {
                action(tnode);
            }
        });
...
private:
    void addRenderizable(core::sptr<Renderizable> newElement);
