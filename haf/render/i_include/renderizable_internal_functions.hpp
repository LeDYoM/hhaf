...
htps::pair<PrimitiveType, htps::size_type> initDataVertexPerFigureAndNumPoints(
    const FigType_t fig_type,
    const htps::size_type num_points) noexcept;

htps::Rects32 textureFillQuad(const htps::sptr<res::ITexture>& texture) noexcept;
...
void updateColors(BasicVertexArray& vertices,Renderizable::RenderizableInternalData const& data);
