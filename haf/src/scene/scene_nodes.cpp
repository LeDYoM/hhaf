...
bool SceneNodes::removeSceneNode(sptr<SceneNode> const& element)
{
    LogAsserter::log_assert(element != nullptr,
                            "Received empty scene node to be deleted");
    LogAsserter::log_assert(m_scene_node != element.get(),
                            "Cannot delete myself from myself");
    LogAsserter::log_assert(
        m_scene_node == element->parent(),
        " You must call removeSceneNode from the parent node");

    auto const old_size{m_scene_nodes.size()};
    m_scene_nodes.erase_one(element);
    return old_size == (m_scene_nodes.size() + 1U);
}

u32 SceneNodes::removeSceneNodes(const SceneNodeVector& element)
{
    u32 result{0U};

    for (const auto& sceneNode : element)
    {
        if (removeSceneNode(sceneNode))
        {
            ++result;
        }
    }
...
bool SceneNodes::removeSceneNodeWithComponent(component::Component& component)
{
    return removeSceneNode(getShared(component.attachedNode()));
}

bool SceneNodes::autoRemove()
{
    auto this_parent{m_scene_node->parent()};
    LogAsserter::log_assert(this_parent != nullptr,
                            "Error removing a node without parent");
    auto this_shared{this_parent->getShared(m_scene_node)};
...
bool SceneNodes::moveToLastPosition(htps::sptr<SceneNode> const& node)
{
    ...
    if (!m_scene_nodes.empty())
    {
        ...
        auto node_iterator{m_scene_nodes.find(node)};

        // If we found the node and it is not the last one
        if (node_iterator != m_scene_nodes.end() &&
            *node_iterator != m_scene_nodes.back())
        {
            std::swap(*node_iterator, m_scene_nodes.back());
            return true;
        }
    }
...
bool SceneNodes::moveToFirstPosition(htps::sptr<SceneNode> const& node)
{
    ...
    if (!m_scene_nodes.empty())
    {
        ...
        // If we found the node and it is not the last one
        if (node_iterator != m_scene_nodes.end() &&
            *node_iterator != m_scene_nodes.front())
        {
            std::swap(*node_iterator, m_scene_nodes.front());
            return true;
        }
    }
