...
public:
    ...
    /// Automatically perform an update of the elements of the current
    /// container using a functor. The functor will be called once per
    /// element in the current container after updating the pending elements
    /// to add and remove.
    /// @tparam F Type of the functor passed as parameter
    /// @param[in] f Functor to be used for each element
    template <typename F>
    void performUpdate(F&& f)
    {
        update();

        for_each_all(main_container_, [f](auto &element) {
            f(element);
        });
