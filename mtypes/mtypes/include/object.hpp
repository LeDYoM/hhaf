...
template <typename T>
constexpr bool storable_or_copyable_as_value_v = storable_as_value_v<T> || is_object_v<T>;
...
public:
    ...
    public:
        ...
        private : const Object *m_object{nullptr};
...
inline Object &operator<<(Object &obj, const vector<T> &data)
{
    ...
    for (const auto &element : data)
    {
        ...
        else if constexpr (std::is_enum_v<T>)
        {
            obj.set(
                counter++,
                static_cast<std::underlying_type_t<T>>(element));
        }
        else
        {
            Object temp;
